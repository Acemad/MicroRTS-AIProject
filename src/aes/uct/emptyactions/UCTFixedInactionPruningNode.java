...
public class UCTFixedInactionPruningNode {
    ...
    private UCTFixedInactionPruningNode parent = null;
    private List<UCTFixedInactionPruningNode> children = null;
...
//    private PlayerActionGenerator actionGenerator = null;
     ...
     * @throws Exception
     */
    public UCTFixedInactionPruningNode(GameState gameState, UCTFixedInactionPruningNode parent, int player) throws Exception {
     ...
     */
    public UCTFixedInactionPruningNode selectLeaf(int player, long cutOffTime, int depthLimit, double evaluationBound, int allowedInactions) throws Exception {
        ...
        if (actionGenerator != null) {
            ...
            if (hasMoreActions) {
                PlayerAction action = actionGenerator.getNextInactionFilteredAction(cutOffTime, allowedInactions); // Normal Fixed Filtering.
                if (action != null) { //Create a new node and add it to the tree. (Expansion)
                    GameState newGameState = gameState.cloneIssue(action);
                    UCTFixedInactionPruningNode newNode = new UCTFixedInactionPruningNode(newGameState.clone(), this, player);
        ...
        for (UCTFixedInactionPruningNode child : children) {
        ...
        return bestChild.selectLeaf(player, cutOffTime, depthLimit, evaluationBound, allowedInactions); // Recursively chose best node in each ply.
    }
     ...
     */
    public double childNodeUCBValue(UCTFixedInactionPruningNode node, double evaluationBound) {
    ...
    public UCTFixedInactionPruningNode getParent() {
        return parent;
    }

    public List<UCTFixedInactionPruningNode> getChildren() {
        return children;
    }
