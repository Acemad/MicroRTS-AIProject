...
public class UCTRandomPruningFixedNode {
    ...
    private UCTRandomPruningFixedNode parent = null;
    private List<UCTRandomPruningFixedNode> children = null;
...
//    private PlayerActionGenerator actionGenerator = null;
     ...
     * @throws Exception
     */
    public UCTRandomPruningFixedNode(GameState gameState, UCTRandomPruningFixedNode parent, int player) throws Exception {
     ...
     */
    public UCTRandomPruningFixedNode selectLeaf(int player, long cutOffTime, int depthLimit, double evaluationBound, int allowedActions) throws Exception {
        ...
        if (actionGenerator != null) {
            ...
            if (hasMoreActions) {
                PlayerAction action = actionGenerator.getNextActionRandomlyPrunedFixed(cutOffTime, allowedActions); // Random Pruning
//                System.out.println(action);
                if (action != null) { //Create a new node and add it to the tree. (Expansion)
                    GameState newGameState = gameState.cloneIssue(action);
                    UCTRandomPruningFixedNode newNode = new UCTRandomPruningFixedNode(newGameState.clone(), this, player);
        ...
        for (UCTRandomPruningFixedNode child : children) {
        ...
        return bestChild.selectLeaf(player, cutOffTime, depthLimit, evaluationBound, allowedActions); // Recursively chose best node in each ply.
    }
     ...
     */
    public double childNodeUCBValue(UCTRandomPruningFixedNode node, double evaluationBound) {
    ...
    public UCTRandomPruningFixedNode getParent() {
        return parent;
    }

    public List<UCTRandomPruningFixedNode> getChildren() {
        return children;
    }
