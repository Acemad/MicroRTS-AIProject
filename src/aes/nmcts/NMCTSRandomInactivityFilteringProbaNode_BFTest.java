...
public class NMCTSRandomInactivityFilteringProbaNode_BFTest {
    ...
    private NMCTSRandomInactivityFilteringProbaNode_BFTest parent; // the node's parent, if any
    ...
    private List<NMCTSRandomInactivityFilteringProbaNode_BFTest> children; // the list of this node's children
    ...
    private HashMap<BigInteger, NMCTSRandomInactivityFilteringProbaNode_BFTest> childrenMap = new LinkedHashMap<>(); // Maps a binInteger to a node
     ...
     * @throws Exception
     */
    public NMCTSRandomInactivityFilteringProbaNode_BFTest(int player, GameState gameState, NMCTSRandomInactivityFilteringProbaNode_BFTest parent, int nodeID, boolean exploreNonSampledActions) throws Exception {
        this.parent = parent;
     ...
     * @throws Exception
     */
    public NMCTSRandomInactivityFilteringProbaNode_BFTest selectLeaf(int player, float epsilon0, float epsilonGlobal, float epsilonLocal, int globalStrategy,
        ...
        if (children.size() > 0 && random.nextFloat() >= epsilon0) {
            ...
            NMCTSRandomInactivityFilteringProbaNode_BFTest selected = null;
     ...
     * @return
     */
    private NMCTSRandomInactivityFilteringProbaNode_BFTest selectFromGlobalMABEpsilonGreedy(float epsilonGlobal) {

        NMCTSRandomInactivityFilteringProbaNode_BFTest best = null;

        if (random.nextFloat() >= epsilonGlobal) { // Exploit : choose the best child.
            for (NMCTSRandomInactivityFilteringProbaNode_BFTest child : children) {
                if (type == 0) { // Max Node
                    if (best == null ||
     ...
     * @return
     */
    private NMCTSRandomInactivityFilteringProbaNode_BFTest selectFromGlobalMABUCB1(float C, double evaluationBound) {
        NMCTSRandomInactivityFilteringProbaNode_BFTest best = null;
        double bestScore = 0;
        for (NMCTSRandomInactivityFilteringProbaNode_BFTest child : children) {
     ...
     * @return
     */
    private NMCTSRandomInactivityFilteringProbaNode_BFTest selectFromLocalMAB(int player, float epsilon0, float epsilonGlobal, float epsilonLocal,
...
//        System.out.println(playerAction);
        ...
        NMCTSRandomInactivityFilteringProbaNode_BFTest oldChild = childrenMap.get(playerActionCode);
        if (oldChild == null) { // If no node with the same playerActionCode exists, create one.
            ...
            NMCTSRandomInactivityFilteringProbaNode_BFTest newChild = new NMCTSRandomInactivityFilteringProbaNode_BFTest(player, newGameState.clone(), this, nodeID, exploreNonSampledActions);
     ...
     * @param child
     */
    public void backpropagate(double evaluation, NMCTSRandomInactivityFilteringProbaNode_BFTest child) {
    ...
    public List<NMCTSRandomInactivityFilteringProbaNode_BFTest> getChildren() {
        return children;
    }
    ...
    public NMCTSRandomInactivityFilteringProbaNode_BFTest getParent() {
        return parent;
    }

    public List<BigInteger> getRejectedActions() {
