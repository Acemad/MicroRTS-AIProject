...
public class NMCTSNode {
    ...
    private NMCTSNode parent; // the node's parent, if any
    ...
    private List<NMCTSNode> children; // the list of this node's children
     ...
     * @throws Exception
     */
    public NMCTSNode(int player, GameState gameState, NMCTSNode parent, int nodeID, boolean exploreNonSampledActions) throws Exception {
        this.parent = parent;
     ...
     * @throws Exception
     */
    public NMCTSNode selectLeaf(int player, float epsilon0, float epsilonGlobal, float epsilonLocal, int globalStrategy,
                                int maxDepth, int nodeID, double evaluationBound) throws Exception {
     ...
     * @return
     */
    private NMCTSNode selectFromGlobalMABEpsilonGreedy(float epsilonGlobal) {

        NMCTSNode best = null;

        if (random.nextFloat() >= epsilonGlobal) { // Exploit : choose the best child.
            for (NMCTSNode child : children) {
                if (type == 0) { // Max Node
                    if (best == null ||
     ...
     * @return
     */
    private NMCTSNode selectFromGlobalMABUCB1(float C, double evaluationBound) {
        NMCTSNode best = null;
        double bestScore = 0;
        for (NMCTSNode child : children) {
            ...
            double evaluation = C * exploitationTerm + explorationTerm;
     ...
     * @return
     */
    private NMCTSNode selectFromLocalMAB(int player, float epsilon0, float epsilonGlobal, float epsilonLocal,
                                         int globalStrategy, int maxDepth, int nodeID, double evaluationBound ) throws Exception {
        ...
        while (!toSample.isEmpty()) { // Loop through the units.
            ...
            try {
                ...
                // Try at random
                int unitActionCode = Sampler.weighted(distribution);
                UnitAction unitAction = element.actions.get(unitActionCode);
                ResourceUsage unitActionResourceUsage = unitAction.resourceUsage(element.unit, gameState.getPhysicalGameState());

                // Check consistency of the unit action with the playerAction.
                if (!playerAction.getResourceUsage().consistentWith(unitActionResourceUsage, gameState)) {
                    ...
                    do { // Loop through the individual unit actions.
                        // Find the index of the inconsistent unit action and remove it from distributionList and outputs.
                        int unitActonIndex = distributionOutputs.indexOf(unitActionCode);
                        distributionList.remove(unitActonIndex);
                        distributionOutputs.remove(unitActonIndex);
        ...
        return oldChild.selectLeaf(player, epsilon0, epsilonGlobal, epsilonLocal, globalStrategy, maxDepth, nodeID, evaluationBound);
    }
     ...
     * @param child
     */
    public void backpropagate(double evaluation, NMCTSNode child) {
    ...
    public List<NMCTSNode> getChildren() {
        return children;
    }
