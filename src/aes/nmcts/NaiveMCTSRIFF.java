 ...
 * @author santi
 */
public class NaiveMCTSRIFF extends AIWithComputationBudget implements InterruptibleAI {
    ...
    protected NaiveMCTSRIFFNode tree;
    ...
    public int global_strategy = NaiveMCTSRIFFNode.E_GREEDY;
    ...
    public NaiveMCTSRIFF(UnitTypeTable utt, int maxAllowedInactions) {
        this(utt);
        this.maxAllowedInactions = maxAllowedInactions;
    }

    public NaiveMCTSRIFF(UnitTypeTable utt) {
        this(100,-1,100,10,
             0.3f, 0.0f, 0.4f,
             new RandomBiasedAI(),
    ...
    public NaiveMCTSRIFF(int available_time, int max_playouts, int lookahead, int max_depth,
    ...
    public NaiveMCTSRIFF(int available_time, int max_playouts, int lookahead, int max_depth, float e_l, float e_g, float e_0, AI policy, EvaluationFunction a_ef, boolean fensa) {
        super(available_time, max_playouts);
        MAXSIMULATIONTIME = lookahead;
        playoutPolicy = policy;
        MAX_TREE_DEPTH = max_depth;
    ...
    public NaiveMCTSRIFF(int available_time, int max_playouts, int lookahead, int max_depth, float e_l, float e_g, float e_0, int a_global_strategy, AI policy, EvaluationFunction a_ef, boolean fensa) {
        super(available_time, max_playouts);
        MAXSIMULATIONTIME = lookahead;
        playoutPolicy = policy;
        MAX_TREE_DEPTH = max_depth;
    ...
    public AI clone() {
        return new NaiveMCTSRIFF(TIME_BUDGET, ITERATIONS_BUDGET, MAXSIMULATIONTIME, MAX_TREE_DEPTH, epsilon_l, discount_l, epsilon_g, discount_g, epsilon_0, discount_0, playoutPolicy, ef, forceExplorationOfNonSampledActions);
    }    
    ...
    public void startNewComputation(int a_player, GameState gs) throws Exception {
        player = a_player;
        current_iteration = 0;
        tree = new NaiveMCTSRIFFNode(player, 1-player, gs, null, ef.upperBound(gs), current_iteration++, forceExplorationOfNonSampledActions);
...
//        System.out.println("HL: " + count + " time: " + (System.currentTimeMillis() - start) + " (" + available_time + "," + max_playouts + ")");
    ...
    public boolean iteration(int player) throws Exception {
        ...
        NaiveMCTSRIFFNode leaf = tree.selectLeaf(player, 1-player, epsilon_l, epsilon_g, epsilon_0, global_strategy,
                MAX_TREE_DEPTH, current_iteration++, maxAllowedInactions);
...
//            System.out.println(total_runs + " - " + epsilon_0 + ", " + epsilon_l + ", " + epsilon_g);
    ...
    public PlayerAction getBestActionSoFar() {
        ...
        if (DEBUG>=1) {
            NaiveMCTSRIFFNode best = (NaiveMCTSRIFFNode) tree.children.get(idx);
...
//            }
        ...
        for(int i = 0;i<tree.children.size();i++) {
            NaiveMCTSRIFFNode child = (NaiveMCTSRIFFNode)tree.children.get(i);
            if (DEBUG>=2) {
...
//            if (best == null || (child.accum_evaluation/child.visit_count)>(best.accum_evaluation/best.visit_count)) {
    ...
    public NaiveMCTSRIFFNode getTree() {
        return tree;
    }
