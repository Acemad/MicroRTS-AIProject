...
public class NMCTSRandomInactivityFilteringFixedNode_BFTest {
    ...
    private NMCTSRandomInactivityFilteringFixedNode_BFTest parent; // the node's parent, if any
    ...
    private List<NMCTSRandomInactivityFilteringFixedNode_BFTest> children; // the list of this node's children
    private double accumulatedEvaluation = 0; // the accumulated evaluation of this node
    private int visitCount = 0; // the visit count
    private PlayerActionGenerator actionGenerator; // the action generator object
    ...
    private HashMap<BigInteger, NMCTSRandomInactivityFilteringFixedNode_BFTest> childrenMap = new LinkedHashMap<>(); // Maps a binInteger to a node
    ...
    private int inactionsCounter = 0;
     ...
     * @throws Exception
     */
    public NMCTSRandomInactivityFilteringFixedNode_BFTest(int player, GameState gameState, NMCTSRandomInactivityFilteringFixedNode_BFTest parent, int nodeID, boolean exploreNonSampledActions) throws Exception {
        this.parent = parent;
     ...
     * @throws Exception
     */
    public NMCTSRandomInactivityFilteringFixedNode_BFTest selectLeaf(int player, float epsilon0, float epsilonGlobal, float epsilonLocal, int globalStrategy,
                                                                     int maxDepth, int nodeID, double evaluationBound, int maxAllowedInactions) throws Exception {
        ...
        if (children.size() > 0 && random.nextFloat() >= epsilon0) {
            ...
            NMCTSRandomInactivityFilteringFixedNode_BFTest selected = null;
            ...
            return selected.selectLeaf(player, epsilon0, epsilonGlobal, epsilonLocal, globalStrategy, maxDepth, nodeID,
                    evaluationBound, maxAllowedInactions);
        }
     ...
     * @return
     */
    private NMCTSRandomInactivityFilteringFixedNode_BFTest selectFromGlobalMABEpsilonGreedy(float epsilonGlobal) {

        NMCTSRandomInactivityFilteringFixedNode_BFTest best = null;

        if (random.nextFloat() >= epsilonGlobal) { // Exploit : choose the best child.
            for (NMCTSRandomInactivityFilteringFixedNode_BFTest child : children) {
                if (type == 0) { // Max Node
                    if (best == null ||
     ...
     * @return
     */
    private NMCTSRandomInactivityFilteringFixedNode_BFTest selectFromGlobalMABUCB1(float C, double evaluationBound) {
        NMCTSRandomInactivityFilteringFixedNode_BFTest best = null;
        double bestScore = 0;
        for (NMCTSRandomInactivityFilteringFixedNode_BFTest child : children) {
     ...
     * @return
     */
    private NMCTSRandomInactivityFilteringFixedNode_BFTest selectFromLocalMAB(int player, float epsilon0, float epsilonGlobal, float epsilonLocal,
                                                                              int globalStrategy, int maxDepth, int nodeID, double evaluationBound,
                                                                              int maxAllowedInactions) throws Exception {
        ...
        while (!toSample.isEmpty()) { // Loop through the units.
            ...
            try {
                ...
                int noneActionCode = element.actionCount - 1;
...
//                System.out.println(element.actions.get(noneActionCode));
                ...
                if (distributionList.size() > 1 && unitAction.getType() == UnitAction.TYPE_NONE &&
                    ...
                    if (inactionsCounter >= maxAllowedInactions || previouslyRejected) {
...
//                        System.out.println(this.nodeID + " Inaction Allowed");
                        inactionsCounter++;
                    }
...
//        System.out.println(playerAction);
        ...
        NMCTSRandomInactivityFilteringFixedNode_BFTest oldChild = childrenMap.get(playerActionCode);
        if (oldChild == null) { // If no node with the same playerActionCode exists, create one.
            ...
            NMCTSRandomInactivityFilteringFixedNode_BFTest newChild = new NMCTSRandomInactivityFilteringFixedNode_BFTest(player, newGameState.clone(), this, nodeID, exploreNonSampledActions);
        ...
        return oldChild.selectLeaf(player, epsilon0, epsilonGlobal, epsilonLocal, globalStrategy, maxDepth, nodeID, evaluationBound, maxAllowedInactions);
    }
     ...
     * @param child
     */
    public void backpropagate(double evaluation, NMCTSRandomInactivityFilteringFixedNode_BFTest child) {
    ...
    public PlayerActionGenerator getActionGenerator() {
    ...
    public List<NMCTSRandomInactivityFilteringFixedNode_BFTest> getChildren() {
        return children;
    }
    ...
    public NMCTSRandomInactivityFilteringFixedNode_BFTest getParent() {
        return parent;
    }
}
