 ...
 * @author santi
 */
public class NaiveMCTSRIFFNode extends MCTSNode {
    ...
    HashMap<BigInteger, NaiveMCTSRIFFNode> childrenMap = new LinkedHashMap<>();    // associates action codes with children
    ...
    // RIFF
    int inactionsCounter = 0;
    List<BigInteger> rejectedActions = new LinkedList<>();


    public NaiveMCTSRIFFNode(int maxplayer, int minplayer, GameState a_gs, NaiveMCTSRIFFNode a_parent, double a_evaluation_bound, int a_creation_ID, boolean fensa) throws Exception {
        parent = a_parent;
        gs = a_gs;
    ...
    // Naive Sampling:
    public NaiveMCTSRIFFNode selectLeaf(int maxplayer, int minplayer, float epsilon_l, float epsilon_g, float epsilon_0,
                                        int global_strategy, int max_depth, int a_creation_ID, int maxAllowedInactions) throws Exception {
        ...
        */
        ...
        if (children.size()>0 && r.nextFloat()>=epsilon_0) {
            ...
            NaiveMCTSRIFFNode selected = null; // Exploit : Global MAB
            ...
            return selected.selectLeaf(maxplayer, minplayer, epsilon_l, epsilon_g, epsilon_0, global_strategy, max_depth, a_creation_ID, maxAllowedInactions);
        }  else {
            // Explore : using Local MABs
            // sample from the local MABs (this might recursively call "selectLeaf" internally):
            return selectLeafUsingLocalMABs(maxplayer, minplayer, epsilon_l, epsilon_g, epsilon_0, global_strategy, max_depth, a_creation_ID, maxAllowedInactions);
        }
    }
   

    
    public NaiveMCTSRIFFNode selectFromAlreadySampledEpsilonGreedy(float epsilon_g) throws Exception {
        if (r.nextFloat()>=epsilon_g) { // Exploit
            NaiveMCTSRIFFNode best = null;
            for(MCTSNode child:children) {
                if (type==0) {
                    // max node:
                    if (best==null || (child.accum_evaluation/child.visit_count)>(best.accum_evaluation/best.visit_count)) {
                        best = (NaiveMCTSRIFFNode)child;
                    }                    
                } else {
                    // min node:
                    if (best==null || (child.accum_evaluation/child.visit_count)<(best.accum_evaluation/best.visit_count)) {
            ...
            NaiveMCTSRIFFNode best = (NaiveMCTSRIFFNode)children.get(r.nextInt(children.size()));
            return best;
        }
    }
    
    
    public NaiveMCTSRIFFNode selectFromAlreadySampledUCB1(float C) throws Exception {
        NaiveMCTSRIFFNode best = null;
        double bestScore = 0;
        for(MCTSNode child:children) {
    ...
    public NaiveMCTSRIFFNode selectLeafUsingLocalMABs(int maxplayer, int minplayer, float epsilon_l, float epsilon_g, float epsilon_0,
                                                      int global_strategy, int max_depth, int a_creation_ID, int maxAllowedInactions) throws Exception {
...
//                System.out.println(ate.actions.get(ate.nactions - 1));
                ...
                // Filtering (1)
                int noneActionCode = ate.nactions - 1;
                boolean inaction = false;
                /*BigInteger rejectedActionCode;
                boolean previouslyRejectedAction = false;*/

                if (distribution.length > 1 && ua.getType() == UnitAction.TYPE_NONE && maxAllowedInactions != -1) {
                    /*// Rejection
                    rejectedActionCode = BigInteger.valueOf(code).multiply(multipliers[i]);
                    if (!rejectedActions.contains(rejectedActionCode))
                        rejectedActions.add(rejectedActionCode);
                    else
                        previouslyRejectedAction = true;*/

                    if (inactionsCounter >= maxAllowedInactions /*|| previouslyRejectedAction*/) // Limit reached, prune
                        inaction = true;
                    else // Allow
                        inactionsCounter++;
                }
                ...
                if (!playerAction.getResourceUsage().consistentWith(r2, gs) || inaction) {
                    ...
                    do {
                        ...
                        if (!dist_l.isEmpty()) {
                            ...
                            code = noneActionCode;
                            ua = new UnitAction(UnitAction.TYPE_NONE, 10);
                        }

                        /*previouslyRejectedAction = false;*/

                        // Filtering (2)
                        if (dist_l.size() > 1 && ua.getType() == UnitAction.TYPE_NONE && maxAllowedInactions != -1) {
                            /*// Rejection
                            rejectedActionCode = BigInteger.valueOf(code).multiply(multipliers[i]);
                            if (!rejectedActions.contains(rejectedActionCode))
                                rejectedActions.add(rejectedActionCode);
                            ...
                            if (inactionsCounter >= maxAllowedInactions /*|| previouslyRejectedAction*/) {
                                idx = dist_outputs.indexOf(code);
                                dist_l.remove(idx);
                                dist_outputs.remove(idx);
        ...
        NaiveMCTSRIFFNode node = childrenMap.get(actionCode);
        if (node == null) {
            actions.add(playerAction);
            GameState gs2 = gs.cloneIssue(playerAction);
            NaiveMCTSRIFFNode newNode = new NaiveMCTSRIFFNode(maxplayer, minplayer, gs2.clone(), this, evaluation_bound, a_creation_ID, forceExplorationOfNonSampledActions);
            childrenMap.put(actionCode,newNode);
            children.add(newNode);
            return newNode;
        }

        return node.selectLeaf(maxplayer, minplayer, epsilon_l, epsilon_g, epsilon_0, global_strategy, max_depth, a_creation_ID, maxAllowedInactions);
    }
    ...
    public void propagateEvaluation(double evaluation, NaiveMCTSRIFFNode child) {
...
//        if (child!=null) System.out.println(evaluation);
        ...
        if (parent != null) {
            ((NaiveMCTSRIFFNode)parent).propagateEvaluation(evaluation, this);
        }
    }
