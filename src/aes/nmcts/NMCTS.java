...
public class NMCTS extends AIWithComputationBudget implements InterruptibleAI {

    // Members ************************************************************************************
    private GameState initialGameState;
    private EvaluationFunction evaluationFunction;
    private AI playoutPolicy = new RandomBiasedAI();
    private NMCTSNode tree;
     ...
     * @param unitTypeTable
     */
    public NMCTS(UnitTypeTable unitTypeTable) {
     ...
     */
    public NMCTS(int timeBudget, int iterationsBudget, int simulationTime, int maxDepth,
    ...
    public void startNewComputation(int player, GameState gameState) throws Exception {
        ...
        tree = new NMCTSNode(player, gameState, null, currentIteration++, exploreNonSampledActions);
    ...
    private boolean monteCarloRun(int player) throws Exception {
        ...
        NMCTSNode selected = tree.selectLeaf(player, epsilon0, epsilonGlobal, epsilonLocal, globalStrategy, maxDepth,
                currentIteration++, evaluationBound);
    ...
    private int getMostVisitedActionIndex() {
        ...
        NMCTSNode mostVisitedChild = null; // The most visited child
        ...
        for (int index = 0; index < tree.getChildren().size(); index++) {
            NMCTSNode child = tree.getChildren().get(index);
    ...
    private int getHighestEvaluatedActionIndex() {
        ...
        NMCTSNode highestEvaluatedChild = null; // The highest evaluated child.
    ...
    @Override
    public AI clone() {
        return new NMCTS(TIME_BUDGET, ITERATIONS_BUDGET, simulationTime, maxDepth,
