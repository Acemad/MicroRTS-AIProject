...
public class NMCTSRandomInactivityFilteringFixedNode {
    ...
    private NMCTSRandomInactivityFilteringFixedNode parent; // the node's parent, if any
    ...
    private List<NMCTSRandomInactivityFilteringFixedNode> children; // the list of this node's children
    ...
    private HashMap<BigInteger, NMCTSRandomInactivityFilteringFixedNode> childrenMap = new LinkedHashMap<>(); // Maps a binInteger to a node
     ...
     * @throws Exception
     */
    public NMCTSRandomInactivityFilteringFixedNode(int player, GameState gameState, NMCTSRandomInactivityFilteringFixedNode parent, int nodeID, boolean exploreNonSampledActions) throws Exception {
        this.parent = parent;
     ...
     * @throws Exception
     */
    public NMCTSRandomInactivityFilteringFixedNode selectLeaf(int player, float epsilon0, float epsilonGlobal, float epsilonLocal, int globalStrategy,
        ...
        if (children.size() > 0 && random.nextFloat() >= epsilon0) {
            ...
            NMCTSRandomInactivityFilteringFixedNode selected = null;
     ...
     * @return
     */
    private NMCTSRandomInactivityFilteringFixedNode selectFromGlobalMABEpsilonGreedy(float epsilonGlobal) {

        NMCTSRandomInactivityFilteringFixedNode best = null;

        if (random.nextFloat() >= epsilonGlobal) { // Exploit : choose the best child.
            for (NMCTSRandomInactivityFilteringFixedNode child : children) {
                if (type == 0) { // Max Node
                    if (best == null ||
     ...
     * @return
     */
    private NMCTSRandomInactivityFilteringFixedNode selectFromGlobalMABUCB1(float C, double evaluationBound) {
        NMCTSRandomInactivityFilteringFixedNode best = null;
        double bestScore = 0;
        for (NMCTSRandomInactivityFilteringFixedNode child : children) {
     ...
     * @return
     */
    private NMCTSRandomInactivityFilteringFixedNode selectFromLocalMAB(int player, float epsilon0, float epsilonGlobal, float epsilonLocal,
...
//        System.out.println(playerAction);
        ...
        NMCTSRandomInactivityFilteringFixedNode oldChild = childrenMap.get(playerActionCode);
        if (oldChild == null) { // If no node with the same playerActionCode exists, create one.
            ...
            NMCTSRandomInactivityFilteringFixedNode newChild = new NMCTSRandomInactivityFilteringFixedNode(player, newGameState.clone(), this, nodeID, exploreNonSampledActions);
     ...
     * @param child
     */
    public void backpropagate(double evaluation, NMCTSRandomInactivityFilteringFixedNode child) {
    ...
    public List<NMCTSRandomInactivityFilteringFixedNode> getChildren() {
        return children;
    }
