...
public class NMCTSRandomInactivityFilteringProbaNode {
    ...
    private NMCTSRandomInactivityFilteringProbaNode parent; // the node's parent, if any
    ...
    private List<NMCTSRandomInactivityFilteringProbaNode> children; // the list of this node's children
    ...
    private HashMap<BigInteger, NMCTSRandomInactivityFilteringProbaNode> childrenMap = new LinkedHashMap<>(); // Maps a binInteger to a node
     ...
     * @throws Exception
     */
    public NMCTSRandomInactivityFilteringProbaNode(int player, GameState gameState, NMCTSRandomInactivityFilteringProbaNode parent, int nodeID, boolean exploreNonSampledActions) throws Exception {
        this.parent = parent;
     ...
     * @throws Exception
     */
    public NMCTSRandomInactivityFilteringProbaNode selectLeaf(int player, float epsilon0, float epsilonGlobal, float epsilonLocal, int globalStrategy,
        ...
        if (children.size() > 0 && random.nextFloat() >= epsilon0) {
            ...
            NMCTSRandomInactivityFilteringProbaNode selected = null;
     ...
     * @return
     */
    private NMCTSRandomInactivityFilteringProbaNode selectFromGlobalMABEpsilonGreedy(float epsilonGlobal) {

        NMCTSRandomInactivityFilteringProbaNode best = null;

        if (random.nextFloat() >= epsilonGlobal) { // Exploit : choose the best child.
            for (NMCTSRandomInactivityFilteringProbaNode child : children) {
                if (type == 0) { // Max Node
                    if (best == null ||
     ...
     * @return
     */
    private NMCTSRandomInactivityFilteringProbaNode selectFromGlobalMABUCB1(float C, double evaluationBound) {
        NMCTSRandomInactivityFilteringProbaNode best = null;
        double bestScore = 0;
        for (NMCTSRandomInactivityFilteringProbaNode child : children) {
     ...
     * @return
     */
    private NMCTSRandomInactivityFilteringProbaNode selectFromLocalMAB(int player, float epsilon0, float epsilonGlobal, float epsilonLocal,
...
//        System.out.println(playerAction);
        ...
        NMCTSRandomInactivityFilteringProbaNode oldChild = childrenMap.get(playerActionCode);
        if (oldChild == null) { // If no node with the same playerActionCode exists, create one.
            ...
            NMCTSRandomInactivityFilteringProbaNode newChild = new NMCTSRandomInactivityFilteringProbaNode(player, newGameState.clone(), this, nodeID, exploreNonSampledActions);
     ...
     * @param child
     */
    public void backpropagate(double evaluation, NMCTSRandomInactivityFilteringProbaNode child) {
    ...
    public List<NMCTSRandomInactivityFilteringProbaNode> getChildren() {
        return children;
    }
