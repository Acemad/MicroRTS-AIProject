...
public class NMCTSDynamicRIFProba extends AIWithComputationBudget implements InterruptibleAI {

    // Members ************************************************************************************
    private GameState initialGameState;
    private EvaluationFunction evaluationFunction;
    private AI playoutPolicy = new RandomBiasedAI();
    private NMCTSRandomInactivityFilteringProbaNode tree;
    ...
    float allowProbabilityOutnumbers;
    float allowProbabilityOutnumbered;

    public NMCTSDynamicRIFProba(UnitTypeTable unitTypeTable, float allowProbabilityOutnumbers,
                                float allowProbabilityOutnumbered) {
        this(unitTypeTable);
        this.allowProbabilityOutnumbers = allowProbabilityOutnumbers;
        this.allowProbabilityOutnumbered = allowProbabilityOutnumbered;
    }
     ...
     * @param unitTypeTable
     */
    public NMCTSDynamicRIFProba(UnitTypeTable unitTypeTable) {
     ...
     */
    public NMCTSDynamicRIFProba(int timeBudget, int iterationsBudget, int simulationTime, int maxDepth,
    ...
    public void startNewComputation(int player, GameState gameState) throws Exception {
        ...
        tree = new NMCTSRandomInactivityFilteringProbaNode(player, gameState, null, currentIteration++, exploreNonSampledActions);
        ...
        if (nbUnits[player] <= nbUnits[1 - player])
            allowProbability = allowProbabilityOutnumbered;
        else
            allowProbability = allowProbabilityOutnumbers;
    }
    ...
    private boolean monteCarloRun(int player) throws Exception {
        ...
        NMCTSRandomInactivityFilteringProbaNode selected = tree.selectLeaf(player, epsilon0, epsilonGlobal, epsilonLocal,
    ...
    private int getMostVisitedActionIndex() {
        ...
        NMCTSRandomInactivityFilteringProbaNode mostVisitedChild = null; // The most visited child
        ...
        for (int index = 0; index < tree.getChildren().size(); index++) {
            NMCTSRandomInactivityFilteringProbaNode child = tree.getChildren().get(index);
    ...
    private int getHighestEvaluatedActionIndex() {
        ...
        NMCTSRandomInactivityFilteringProbaNode highestEvaluatedChild = null; // The highest evaluated child.
    ...
    @Override
    public AI clone() {
        return new NMCTSDynamicRIFProba(TIME_BUDGET, ITERATIONS_BUDGET, simulationTime, maxDepth,
