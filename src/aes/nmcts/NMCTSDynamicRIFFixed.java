...
public class NMCTSDynamicRIFFixed extends AIWithComputationBudget implements InterruptibleAI {

    // Members ************************************************************************************
    private GameState initialGameState;
    private EvaluationFunction evaluationFunction;
    private AI playoutPolicy = new RandomBiasedAI();
    private NMCTSRandomInactivityFilteringFixedNode tree;
    ...
    int maxAllowedInactions = 0;
    int maxAllowedInactionsOutnumbered;
    int maxAllowedInactionsOutnumbers;

    public NMCTSDynamicRIFFixed(UnitTypeTable unitTypeTable, int maxAllowedInactionsOutnumbers,
                                int maxAllowedInactionsOutnumbered) {
        this(unitTypeTable);
        this.maxAllowedInactionsOutnumbers = maxAllowedInactionsOutnumbers;
        this.maxAllowedInactionsOutnumbered = maxAllowedInactionsOutnumbered;
    }
     ...
     * @param unitTypeTable
     */
    public NMCTSDynamicRIFFixed(UnitTypeTable unitTypeTable) {
        this(100, -1, 100, 10, 0.4f, 0.0f, 0.3f,
                NMCTSRandomInactivityFilteringFixedNode.EPSILON_GREEDY, new RandomBiasedAI(), new SimpleSqrtEvaluationFunction3(), true);
    }
     ...
     */
    public NMCTSDynamicRIFFixed(int timeBudget, int iterationsBudget, int simulationTime, int maxDepth,
    ...
    public void startNewComputation(int player, GameState gameState) throws Exception {
        ...
        tree = new NMCTSRandomInactivityFilteringFixedNode(player, gameState, null, currentIteration++, exploreNonSampledActions);
        ...
        if (nbUnits[player] <= nbUnits[1 - player])
            maxAllowedInactions = maxAllowedInactionsOutnumbered;
        else
            maxAllowedInactions = maxAllowedInactionsOutnumbers;
    }
    ...
    private boolean monteCarloRun(int player) throws Exception {
        ...
        NMCTSRandomInactivityFilteringFixedNode selected = tree.selectLeaf(player, epsilon0, epsilonGlobal, epsilonLocal,
                globalStrategy, maxDepth, currentIteration++, evaluationBound, maxAllowedInactions);
    ...
    private int getMostVisitedActionIndex() {
        ...
        NMCTSRandomInactivityFilteringFixedNode mostVisitedChild = null; // The most visited child
        ...
        for (int index = 0; index < tree.getChildren().size(); index++) {
            NMCTSRandomInactivityFilteringFixedNode child = tree.getChildren().get(index);
    ...
    private int getHighestEvaluatedActionIndex() {
        ...
        NMCTSRandomInactivityFilteringFixedNode highestEvaluatedChild = null; // The highest evaluated child.
    ...
    @Override
    public AI clone() {
        return new NMCTSDynamicRIFFixed(TIME_BUDGET, ITERATIONS_BUDGET, simulationTime, maxDepth,
